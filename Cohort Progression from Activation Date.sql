WITH overall AS (
    SELECT ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY approval_datetime) AS card, 
    loan_loan.* 
    FROM loan_loan 
    WHERE approval_datetime NOTNULL
                ),
...
prog AS (
  SELECT card, 
    LL.user_id, 
    LL.id, 
    FIRST_VALUE(approval_datetime) OVER (PARTITION BY LL.user_id ORDER BY approval_datetime) AS first_loan , 
    approval_datetime,
    principal_defaulted/100 AS principal_defaulted, 
    TT.*
FROM overall LL
LEFT JOIN transaction_transaction TT 
    ON LL.id=TT.loan_id 
    AND status = 2 
